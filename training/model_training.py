...
def load_data(path: str) -> pd.DataFrame:
    ...
    entity_df = pd.read_parquet(
        path, columns=["trip_id", "pickup_datetime", settings.TARGET]
    )
...
def hyperparameter_tuning(X_train: pd.DataFrame, y_train: pd.Series) -> Dict[str, Any]:
    ...
    if fraction < 1.0:
        ...
        sample_idx = X_train.sample(
            frac=fraction, random_state=settings.TUNING_CONFIG["random_state"]
        ).index
...
def evaluate_model(
    model: xgb.XGBRegressor, X_val: pd.DataFrame, y_val: pd.Series
) -> Dict[str, float]:
